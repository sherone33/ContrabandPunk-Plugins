...
// Cap pixel names + reserved-status flag                                                                                                                                
var CAP_NAMES = [                                                                                                                                                        
...
// wrap each so it logs first, then calls the original. The engine's click router                                                                                        
// looks up these names at call-time, so it sees our wrapped versions.
if(typeof toggleEKG === "function") {                                                                                                                                    
   var _origToggleEKG = toggleEKG;                                                                                                                                     
   toggleEKG = function() {                                                                                                                                              
      console.log("Cap pixel 1 clicked: EKG  (Note: this button is reserved by the engine and can't be overridden by a plugin!)");                                     
...
if(typeof toggleEqualizer === "function") {
   ...
   toggleEqualizer = function() {                                                                                                                                        
      console.log("Cap pixel 2 clicked: Equalizer  (Note: this button is reserved by the engine and can't be overridden by a plugin!)");
      return _origToggleEqualizer.apply(this, arguments);                                                                                                                
   };           
}                                                                                                                                                                        
                
function pluginInit(config, canvas, ctx) {
   console.log("Click Logger plugin initialized — now logging all 5 cap-pixel taps");
